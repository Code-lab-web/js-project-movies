...
Socket.prototype.connect = function (port, address, callback) {
  ...
  if (state.bindState !== BIND_STATE_BOUND) {
    enqueue(
      this,
      FunctionPrototypeBind(_connect, this, port, address, callback)
    );
    return;
  }
...
Socket.prototype.disconnect = function () {
  ...
  if (err) throw new ErrnoException(err, "connect");
  else state.connectState = CONNECT_STATE_DISCONNECTED;
};
...
function fixBufferList(list) {
  ...
  for (let i = 0, l = list.length; i < l; i++) {
    ...
    else if (Buffer.isBuffer(buf)) newlist[i] = buf;
...
Socket.prototype.send = function (
  ...
  if (list.length === 0) ArrayPrototypePush(list, Buffer.alloc(0));
...
Socket.prototype.remoteAddress = function () {
  ...
  if (err) throw new ErrnoException(err, "getpeername");
