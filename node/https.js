...
function getTunnelConfigForProxiedHttps(agent, reqOptions) {
  ...
  debug(
    `getTunnelConfigForProxiedHttps should use proxy for ${reqOptions.host}:${reqOptions.port}:`,
    shouldUseProxy
  );
  if (!shouldUseProxy) {
    return null;
...
function createConnection(...args) {
  ...
  debug(
    `https createConnection should use proxy for ${options.host}:${options.port}:`,
    tunnelConfig
  );
...
const globalAgent = new Agent({
  ...
  proxyEnv: getOptionValue("--use-env-proxy")
    ? filterEnvForProxies(process.env)
    : undefined,
});
